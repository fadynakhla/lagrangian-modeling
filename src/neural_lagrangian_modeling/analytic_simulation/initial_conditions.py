...
def figure_eight() -> tuple[datamodels.MassiveBody, datamodels.MassiveBody, datamodels.MassiveBody]:
    ...
    # Initial positions
    r1 = np.array([0.97000436, -0.24308753], dtype=np.float128)
    r2 = np.array([-r1[0], -r1[1]], dtype=np.float128)  # Mirror of r1
    r3 = np.array([0., 0.], dtype=np.float128)  # Center

    # Initial velocities
    v3 = np.array([-0.93240737, -0.86473146], dtype=np.float128)
    v1 = np.array([0.46620369, 0.43236573], dtype=np.float128)  # -v3/2
    v2 = v1  # Same as v1
...
def lagrange_triangle() -> tuple[datamodels.MassiveBody, datamodels.MassiveBody, datamodels.MassiveBody]:
    ...
    r1 = np.array([1., 0.], dtype=np.float128)
    r2 = np.array([-0.5, 0.866025404], dtype=np.float128)  # cos(120°), sin(120°)
    r3 = np.array([-0.5, -0.866025404], dtype=np.float128)  # cos(-120°), sin(-120°)

    # Angular velocity for stable orbit
    omega = 1.0  # Can be adjusted

    # Velocities for circular motion
    v1 = omega * np.array([0., 1.], dtype=np.float128)
    v2 = omega * np.array([-0.866025404, -0.5], dtype=np.float128)
    v3 = omega * np.array([0.866025404, -0.5], dtype=np.float128)
...
def sun_earth_moon() -> tuple[datamodels.MassiveBody, datamodels.MassiveBody, datamodels.MassiveBody]:
    ...
    # Positions
    r_sun = np.array([0., 0.], dtype=np.float128)
    r_earth = np.array([1., 0.], dtype=np.float128)
    r_moon = np.array([1.1, 0.], dtype=np.float128)

    # Velocities (approximately circular orbits)
    v_sun = np.array([0., 0.], dtype=np.float128)
    v_earth = np.array([0., np.sqrt(m_sun)], dtype=np.float128)
    v_moon = np.array([0., np.sqrt(m_sun/1.1) + 0.1], dtype=np.float128)
...
if __name__=="__main__":
    ...
    visualizer.animate_trajectories(
        ...
        save_path="animation_figure.gif"
    )
